...
<template>
	<view>
		...
		<view class="goods-section" v-for="(item, index) in shopGoodsList" :key="index">
			...
			<!-- 商品列表 -->
			<view class="g-item" v-for="(subitem, indexC) in item.goodsList" :key="indexC" v-if="item.showGoodsAll?indexC>=0:indexC<2">
				<view class="goods">
					<view class="img">
						<image :src="subitem.imageUrl || '../../static/goodsImg.jpg'"></image>
						<text v-if="subitem.isFlashGood===1" class="tt_pp" >秒杀</text>
					</view>
					<view class="right">
						<view class="price-box">
							<text class="price">￥{{subitem.price|numberFormat}}</text>
							<text class="number">x {{subitem.num}}</text>
						</view>
						<text class="clamp title">
							<text class="rx" v-if="subitem.isGroupGoods===1">套餐</text>
						...
						<text class="spec" v-if="subitem.name">{{subitem.name}}</text>
						<text class="spec" v-if="subitem.attrNameStr">{{subitem.attrNameStr}}</text>
						<!-- <text class="spec" v-if="subitem.lotNumber">{{subitem.lotNumber}}</text> -->
					</view>
				</view>
				<view class="groupGoods" v-if="subitem.isGroupGoods === 1" v-for="(groupItem,groupsIndex) in subitem.goodsGroups">
					<image :src="groupItem.imageUrl"></image>
					<view class="right">
						<text class="clamp title">
							<text>{{groupItem.name}}</text>
						</text>
						<text class="spec">{{groupItem.manufacturerName}}</text>
						<text class="spec">x {{groupItem.groupNumber}}</text>
					</view>
				</view>

			</view>
			<!--向下箭头 展示全部-->
			<view class="showmore" v-if="!item.showGoodsAll&&item.goodsList.length>2"  @click="doShowAllGoods(index)">
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell desc-cell">
				<view class="clearFlex info_cv">
					<text class="cell-tit clamp">发票是否随货</text>
					<view class=" cell-tip radioView">
						<label class="radio" @click="touchRadio1(1)"><radio class="from-radio" value="1" :checked="billWithOrder === 1" />是</label>
						<label class="radio" @click="touchRadio1(0)"><radio class="from-radio" value="0" :checked="billWithOrder === 0" />否</label>
					</view>
				</view>
				<view class="clearFlex info_cv">
					<text class="cell-tit clamp">发票是否月开</text>
					<view class=" cell-tip radioView">
						<label class="radio" @click="touchRadio2(1)"><radio class="from-radio" value="1" :checked="billMonthOpen === 1" />是</label>
						<label class="radio" @click="touchRadio2(0)"><radio class="from-radio" value="0" :checked="billMonthOpen === 0" />否</label>
					</view>
				</view>
				<view class="clearFlex info_cv" v-if="billWithOrder===1">
					<text class="cell-tit clamp">发票类型</text>
					<view class=" cell-tip radioView">
						<label class="radio" @click="touchRadio3('ordinary')"><radio class="from-radio" value="ordinary" :checked="billMethod === 'ordinary'" />普票</label>
						<label class="radio" @click="touchRadio3('special')"><radio class="from-radio" value="special" :checked="billMethod === 'special'" />专票</label>
					</view>
				</view>
				<view class="clearFlex info_cv info_cv2">
					<text class="cell-tit clamp">备注</text>
					<input class="desc" type="text" v-model="desc" placeholder="选填: 填写内容已和客服协商确认" placeholder-class="placeholder" />
				</view>
			</view>
		</view>
		...
		<view class="clearFlex footer">
			<view class="price-content">
				...
				<text class="price">{{goodMoneyObj.totalMoney}}</text>
			</view>
<!--			<text class="canleBnt" @click="goBack()">取消订单</text>-->
			<text class="submit" @click="isFirstBuy">提交订单</text>
		</view>
	...
	export default {
		data() {
			...
			return {
				...
				shopDeliveryPriceType: 1,
				redPackagePrice: 0,
				redpacketArray: [],
				gameArray: [],
				gameArray2: [],
				totalAward: 0, //总优惠
				billWithOrder:0,      // 开票是否随货
				billMonthOpen:1,      // 开票是否月开
				billMethod:'ordinary',         // 开票方式   ordinary : 普票 special:专票
				shopBId:0,
				option: {}
			}
		},
		...
		onLoad(option){
			...
			if (option.isAgainSubmitOrder) { // 订单详情H5点击再来一单时，处理页面参数
				this.creatOrder(JSON.parse(decodeURIComponent(option.item)))
			}
			option = this.option
			//商品数据
		...
		methods: {
				...
				if (multiShop.length > 0) {
					var totalMoney = 0 //总金额
									...
									try{
										...
										if (utils.isNotNull(goodsInfo)) {
											var info = {
												...
												"attrName": goodsInfo.attrName,
												"attrIdStr": goodsInfo.attrIdStr,
												"attrNameStr": goodsInfo.attrNameStr,
												"weight": utils.isNotNull(goodsInfo.weight) ? goodsInfo.weight : 0,
												"goodsGroups": goodsInfo.goodsGroups,
												"showAllGroup": false,
												"isGroupGoods": goodsInfo.isGroupGood,
												"isFlashGood": goodsInfo.isFlashGood,
												"lotNumber": goodsInfo.lotNumber,
											...
											totalGoodsMoney += (goodsInfo.isFlashGood===1?goodsInfo.flashPrice:goodsInfo.price) * goodsInfo.num;
						...
						var goodDisplayList = [
							...
							{
								'name': '配送费用',
								'price': totalPeisong===0?'快递 免邮':(currencyUnit + totalPeisong.toFixed(2))
							},
							{
								'name': '已优惠',
								'price': currencyUnit + '0.00'
							},
							{
								'name': '实付款',
			...
			dealShopPsType () {
				...
				console.log("配送费用"+showPs +","+shopZq)
			},
			...
			// 判断首营   /user/shopb/order/is-first-buy
			isFirstBuy: function () {
				var that = this;
				var token = uni.getStorageSync("token");
				...
				var url = "/action/user/shopb/order/is-first-buy"
				var params = {
				...
				this.$apis.sendRequest({
					...
					success: function (result) {
						console.log(result)
						if (utils.isNotNull(result.info) && result.info === '1') {
							// 首营 提示
							// #ifdef APP-PLUS
							uni.$showModal({
								// concent: "您还未向厂商提交首营资料,请先提交首营资料!",
								// concent: "您还未和奥瑞森医药交换纸质资质 ，请尽快安排邮寄！谢谢",
								concent: "请补全店铺资质！谢谢",
								cancelVal:'稍后再说',
								confirmVal:'立刻提交',
							}).then(res=>{
								// 提交订单
								that.submitSingleShopOrder()
								//确认
							}).catch(res=>{
								//取消
								console.log(res);
			...
			submitOrderNum(){
				var that = this
				var quota = 0
				if(utils.isNotNull(uni.getStorageSync('shopInfo'))){
					quota = uni.getStorageSync('shopInfo').freeYunFei
				}
				if(that.goodMoneyObj.totalGoodsMoney < quota){
					uni.showModal({
					    title: '提示',
					    content: '您所选购订单总金额还未达到最低包邮（'+ quota +'元起包邮）起配，将会产生邮费！快去选购其他药品吧',
						cancelText: '继续提交',
						confirmText: '选购商品',
					    success: function (res) {
					        if (res.confirm) {
			...
			submitSingleShopOrder() {
				...
				var params = {
				   ...
				   "isAppointment": 0,
				   "integral": "",
					"billWithOrder":that.billWithOrder,      // 开票是否随货
					"billMonthOpen":that.billMonthOpen,      // 开票是否月开
					"billMethod":that.billMethod,         // 开票方式
					"shopBId":that.shopBId				// 关联的药店信息
				 };
				this.$apis.sendRequest({
					url : url,
					data : params,
			...
			moneyCallBack (lng, lat, id) {
				...
				if (utils.isNotNull(shopInfo.freeYunFei) && shopInfo.freeYunFei > 0 ) {
					if (utils.isNotNull(shopInfo.freeYunFeiMoney) && shopInfo.freeYunFeiMoney == -1 && totalGoodsMoney >= freeYunFei) { //免运费
						deliveryPrice = 0;
					}
					else if (utils.isNotNull(shopInfo.freeYunFeiMoney) && shopInfo.freeYunFeiMoney > 0 && totalGoodsMoney >= freeYunFei) { // 满足金额减运费金额
				...
				var totalMoneyNew = (Number(goodMoneyObjVal.totalGoodsMoney) + Number(deliveryPrice)
						- Number(totalAward)
						- Number(redPackagePrice) + Number(urgentPrice) - Number(pledgeMoney));


				totalMoneyNew = totalMoneyNew > 0  ? totalMoneyNew.toFixed(2) : 0.00 ;
				for (let i = 0; i < goodDisplayArrVal.length; i++) {
					if (goodDisplayArrVal[i].name == '配送费用') {
					  goodDisplayArrVal[i].price = deliveryPrice===0?"快递 免邮":"￥" + deliveryPrice.toFixed(2)
					  goodMoneyObjVal.totalPeisong = deliveryPrice
					}
					if (totalAward > 0 && goodDisplayArrVal[i].name == '已优惠') {
						goodDisplayArrVal[i].price = "-￥" + totalAward.toFixed(2)
			...
			touchRadio1(type) {
				var that = this
				that.billWithOrder = type
				type ===1 ? that.billMonthOpen = 0 : that.billMonthOpen = 1
			},
			// 发票是否月开
			touchRadio2(type) {
				var that = this
				that.billMonthOpen = type
				type ===1 ? that.billWithOrder = 0 : that.billWithOrder = 1
			},
			// 发票类型 ordinary : 普票 special:专票
			touchRadio3(type) {
				var that = this
				that.billMethod = type
			},
			doShowAllGoods(index) {
				this.shopGoodsList[index].showGoodsAll = true
			},
			// 再来一单时，处理页面参数
			creatOrder (item) {
				var token = uni.getStorageSync("token")
				...
				for (var i = 0; i < goods.length; i++) {
					...
					var isGroupGood = goodsItem.isGroupGood
					var goodsGroups = []
					var orderGoodsList = goodsItem.orderGoodsList
					if (isGroupGood === 1 && Array.isArray(orderGoodsList)) {
						goodsGroups = orderGoodsList
					}
			
					var gdId = goodsItem.goodsId;
					var spId = goodsItem.goodsSpecsId;
					var attrIdStr = goodsItem.attrIdStr?goodsItem.attrIdStr:'';
					if ((utils.isNotNull(attrIdStr) && attrIdStr) || spId > 0) {
						gdId = gdId + '_' + spId + '_' + attrIdStr
					}else{
						gdId = gdId + '_' + spId + '_'
					...
					uni.setStorage({
					  key: gdId + '', // element.goodsSpecsId > 0 ? element.goodsId + "" + element.goodsSpecsId : element.goodsId + "",
					  data: {
					    id: goodsItem.goodsId,
					    specsId: goodsItem.goodsSpecsId,
					    src: goodsItem.imageUrl,
					    name: goodsItem.name,
					    price: goodsItem.goodsPrice,
					    num: goodsItem.totalCount,
					    packetPrice: goodsItem.packetPrice,
					    attrNameStr: utils.isNotNull(goodsItem.goodsAttrVals) ? goodsItem.goodsAttrVals : '',
					    isGroupGood: goodsItem.isGroupGood,
							goodsGroups: goodsGroups
					  },
					})
					// uni.setStorage({
					// 	key:gdId + '',
					// 	data:JSON.stringify(goodsItem)
					// })
		
					goodsA.push({
						goodsId: goodsItem.goodsId?goodsItem.goodsId:goodsItem.id,
						specsId: utils.isNotNull(goodsItem.goodsSpecsId) && goodsItem.goodsSpecsId > 0 ? goodsItem.goodsSpecsId : 0,
						totalCount: goodsItem.totalCount,
						attrIdStr: utils.isNotNull(goodsItem.attrIdStr) ? goodsItem.attrIdStr : '',
						attrNameStr: utils.isNotNull(goodsItem.attrNameStr) ? goodsItem.attrNameStr : '',
						isGroupGood: goodsItem.isGroupGood
					});
				}
				if (goodsA.length > 0) {
				...
				this.option.goodsArr = goodString
			}
		},
...
<style type="text/less" lang="scss">
	...
	.address-section {
		...
		margin-top: 16upx;
		border-radius:10upx;
		border-bottom: 5upx solid #04C1AB;
		.order-content {
			margin-top: 30upx;
			display: flex;
			align-items: center;

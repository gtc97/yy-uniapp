<template>
	<view>
		<!-- 秒杀商品 -->
		<view class="seckill-price-box seckill-bg"  v-if="detail.isFlashGood===1&&show">
			<view class="x-bc price-top-box">
				<view class="x-f">
					<text class="unit">￥</text>
					<text class="price">{{ detail.flashPrice }}</text>
					<text class="notice">秒杀价</text>
				</view>
				<view class="count-down" v-show="flashSaleInfo.status === 'waiting'">
					距开始 {{ flashSaleInfo.countDownTime.h || '00' }} : {{ flashSaleInfo.countDownTime.m || '00' }} : {{ flashSaleInfo.countDownTime.s || '00' }}
				</view>
				<view class="count-down" v-show="flashSaleInfo.status === 'end'">已结束</view>
				<view class="count-down" v-show="flashSaleInfo.status === 'ing'" v-if="showTime">
					距结束 {{ flashSaleInfo.countDownTime.h || '00' }} : {{ flashSaleInfo.countDownTime.m || '00' }} : {{ flashSaleInfo.countDownTime.s || '00' }}
				</view>
			</view>
			<view class="x-bc price-bottom-box">
				<view class="x-f">
					<view class="original-price">原价：￥{{ detail.price }}</view>
					<text class="line"></text>
					<view class="sold">已售：{{ flashSaleInfo.marketCount }}件</view>
				</view>
				<view class="express">
					<view class="x-f">
						<view class="cu-progress round sm" :class="flashSaleInfo.status !== 'end' ? 'seckill-progress' : 'seckilled-progress'">
							<view
								:class="flashSaleInfo.status !== 'end' ? 'progress-item--noend' : 'progress-item--end'"
								:style="[{ width: loading ? getProgress(flashSaleInfo.goodsCount, flashSaleInfo.marketCount) : '' }]"
							></view>
						</view>
						<view class="progress-text" v-if="flashSaleInfo.stock > 0">仅剩{{ flashSaleInfo.stock }}件</view>
...
export default {
	...
	methods: {
		...
		countDown() {
			let that = this;
			let t = 0;
			// if (that.flashSaleInfo.endTime < new Date().getTime()) {
			// 	that.flashSaleInfo.status = 'end';
			// 	that.$emit('change', JSON.stringify(that.flashSaleInfo));
			// }
			that.countDownDispose(t)
			t--;
			that.timer = setInterval(() => {
				t = that.countDownDispose(t)
				t--;
				// that.$emit('change', JSON.stringify(that.flashSaleInfo));
			}, 1000);
		},
		countDownDispose (t) {
			...
			if (nowTime < that.flashSaleInfo.startTime) {
				that.flashSaleInfo.status = 'waiting';
				t = that.flashSaleInfo.startTime - nowTime;
			} else if (nowTime > that.flashSaleInfo.endTime) {
				that.flashSaleInfo.status = 'end';
				clearInterval(that.timer);
				that.timer = null
				return;
			} else {
				that.flashSaleInfo.status = 'ing';
				t = that.flashSaleInfo.endTime - nowTime;
			}
			that.flashSaleInfo.countDownTime = utils.formatToHours(t / 1000);
			that.show = true
			that.showTime = true
			return t
		}
